...
// Badges
image::https://github.com/LimitlessGreen/Audio-Workbench/actions/workflows/ci.yml/badge.svg[CI]
image::https://img.shields.io/npm/v/audio-workbench.svg[ NPM]
image::https://img.shields.io/pypi/v/audio-workbench.svg[ PyPI]
...
* **Dual spectrogram presets** — Perch (mel + PCEN) and Classic (linear + dB, xeno-canto style)
* **Waveform + spectrogram** rendered side-by-side with synchronized scrolling and zoom
* **Label annotations** — draw, drag, resize time×frequency boxes on the spectrogram; pixel-space dragging feels perceptually linear on mel and linear scales
* **Label taxonomy** — customizable species presets with colors and keyboard shortcuts (`1..9`)
* **Bandpass-filtered playback** — isolate and play back a specific time×frequency region via Web Audio
* **External spectrogram injection** — supply pre-computed Float32 data or a rendered image; bypass internal DSP
* **Settings side-panel** — FFT size, max frequency, color scheme, display gain, auto contrast, zoom
* **Crosshair overlay** — real-time time + frequency readout, mel-aware coordinate display
* **Follow modes** — Free, Follow (jump-to-center), Smooth (continuous scroll)
* **Compact preview modes** — hero transport, overlay mode, hide any UI element for small embeds
* **18 events** — ready, timeupdate, seek, selection, zoomchange, transport state, and more
* **Multi-instance safe** — `data-aw` attributes instead of `id`, CSS scoped to `.daw-shell`
* **Web Worker DSP** — FFT, mel filterbank, and PCEN run off the main thread
* **Performance overlay** — FPS, long frames, event rates, transport transitions
* **ESM + IIFE builds** — Vite library mode with sourcemaps and auto-generated TypeScript declarations
* **Python wrapper** — embed in Streamlit and Jupyter with one function call
* **110 tests** — DSP, spectrogram utils, coordinate system, interaction state, transport state
* **CI / CD** — GitHub Actions for typecheck, test, build; GitHub Pages deployment; Dependabot
...
Plays back only the audio within the given time and frequency range using a Web Audio bandpass filter.
Useful for isolating individual bird calls from a recording.
...
Default taxonomy: Bird Call (`1`), Song (`2`), Chirp (`3`), Noise (`4`).
Fully customizable — set your own species list with colors and keyboard shortcuts.
...
The Classic preset produces spectrograms visually matching https://xeno-canto.org[xeno-canto.org].
...
Toggle the crosshair button to show a real-time overlay displaying the exact time and frequency at the mouse position.
Coordinates are mel-aware — frequency readout matches the current scale (mel or linear).
...
The build uses https://vitejs.dev/[Vite] in library mode:
...
. Verify build + packaging locally:
+
[source,bash]
----
npm test
...
. Publishing runs via GitHub Actions on tag `v*`:
** npm: `publish-npm` job
