 ...
 */
export async function openMapModal({ lat = 51, lon = 10, zoom = 6, readOnly = false, onConfirm, onCancel }) {
  await loadLeaflet();
  ...
  function syncInputs(lt, ln) {
    currentLat = lt;
    currentLon = ln;
    if (latInput) latInput.value = lt.toFixed(5);
    if (lonInput) lonInput.value = ln.toFixed(5);
  }
