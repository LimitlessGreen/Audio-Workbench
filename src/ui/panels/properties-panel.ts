...
const RECORDING_FIELDS = [
  ...
  { key: 'sampleRate',  label: 'Sample rate', fmt: (v: any) => `${v} Hz` },
  { key: 'duration',    label: 'Duration',    fmt: (v: any) => `${Number(v).toFixed(2)} s` },
...
const LABEL_FIELDS = [
  ...
  { key: 'start',            label: 'Start',           edit: 'number', step: 0.001, suffix: 's', fmt: (v: any) => `${Number(v).toFixed(3)} s` },
  { key: 'end',              label: 'End',             edit: 'number', step: 0.001, suffix: 's', fmt: (v: any) => `${Number(v).toFixed(3)} s` },
  { key: 'freqMin',          label: 'Freq min',        edit: 'number', step: 1,     suffix: 'Hz', fmt: (v: any) => v != null ? `${Number(v).toFixed(0)} Hz` : '' },
  { key: 'freqMax',          label: 'Freq max',        edit: 'number', step: 1,     suffix: 'Hz', fmt: (v: any) => v != null ? `${Number(v).toFixed(0)} Hz` : '' },
  { key: 'confidence',       label: 'Confidence',      fmt: (v: any) => v != null ? `${(Number(v) * 100).toFixed(1)}%` : '' },
...
export class PropertiesPanel {
   ...
   */
  setRecordingMeta(meta: any, audioInfo: any) {
   ...
   */
  setSnr(id: any, status: any, value: any) {
  ...
  _renderRecording() {
    ...
    this._recBody.appendChild(
      this._buildReadonlyGrid(RECORDING_FIELDS, merged, (hasCoords ? { lat, lon } : null) as any)
    );
  }
   ...
   */
  _patchValues(lbl: any, editable: any) {
   ...
   */
  _buildReadonlyGrid(fields: any, data: any, mapCoords = null) {
    const dl = document.createElement('dl');
    dl.className = 'props-grid';
    for (const f of fields) {
      ...
      if (mapCoords && (f.key === 'lat' || f.key === 'lng')) {
        ...
        if (f.key === 'lat') {
          ...
          mapBtn.title = `Show on map (${(mapCoords as any).lat.toFixed(4)}, ${(mapCoords as any).lon.toFixed(4)})`;
          mapBtn.innerHTML = GEO_ICONS.map;
          mapBtn.addEventListener('click', () =>
            openMapModal({ lat: (mapCoords as any).lat, lon: (mapCoords as any).lon, zoom: 10, readOnly: true })
          );
  ...
  _buildLabelGrid(lbl: any, editable: any) {
    const dl = document.createElement('dl');
    dl.className = 'props-grid';
  ...
  _buildInput(field: any, value: any, lbl: any) {
    if (field.edit === 'color') {
    ...
    if (field.key === 'label' && this._speciesSearchFactory) {
      ...
      const widget = this._speciesSearchFactory(placeholder, ({ name, scientificName }: any) => {
  ...
  _buildNumberInput(field: any, value: any, lbl: any) {
  ...
  _buildColorInput(field: any, value: any, lbl: any) {
    const wrap = document.createElement('span');
  ...
  _buildTagsSection(lbl: any, editable: any) {
  ...
  _emitChange(id: any, updates: any) {
    this.onChange?.(id, updates);
  }

  /** Commit and blur when Enter is pressed. */
  _bindEnterBlur(input: any) {
    input.addEventListener('keydown', (e: any) => { if (e.key === 'Enter') { e.preventDefault(); input.blur(); } });
  }
