...
import { PERCH_FRAME_RATE, DEFAULT_SAMPLE_RATE, CQT_FMIN, CQT_BINS_PER_OCTAVE } from '../shared/constants.ts';
import { buildMelFrequencies, buildCQTFrequencies, hzToMel, melToHz } from './dsp.ts';
import { clamp } from '../shared/utils.ts';

export interface ComputeMaxBinParams {
    isLinear: boolean;
    nyquist: number;
    spectrogramMels: number;
    boundedMaxFreq: number;
    melFreqs: Float32Array | null;
}

export function computeMaxBin({ isLinear, nyquist, spectrogramMels, boundedMaxFreq, melFreqs }: ComputeMaxBinParams): number {
    if (isLinear) {
...
export interface CoordinateSystemParams {
    ...
    freqViewportMin?: number | null;
    freqViewportMax?: number | null;
    axisWidth?: number;
}
