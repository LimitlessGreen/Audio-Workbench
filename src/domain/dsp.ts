 ...
 */
export function buildMelFrequencies(sampleRate: number, nMels: number): Float32Array {
...
export function createMelFilterbank(sampleRate: number, fftSize: number, nMels: number, fMin: number, fMax: number): Float32Array[] {
...
export function applyMelFilterbank(powerSpectrum: Float32Array, melFilterbank: Float32Array[]): Float32Array {
...
export function iterativeFFT(real: Float32Array, imag: Float32Array): void {
    const n = real.length;
 ...
 */
export function iecDbToFader(db: number): number {
    if (db < -70) return 0;
