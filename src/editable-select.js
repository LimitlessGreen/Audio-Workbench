...
export function createEditableSelect(opts) {
  ...
  function selectValue(val) { currentValue = val; updateTriggerText(); hide(); opts.onChange(val); }
  ...
  return {
    ...
    setValue(val) { currentValue = val || ''; updateTriggerText(); },
    setItems(newItems) { items = (newItems || []).slice(); if (open) renderList(); },
